...
def main(_user, _passwd, _step):
    ...
    # print(response)
    result = f"{_user[:3]}****{_user[-4:]}: [{now}] 修改步数（{_step}）" + response["message"]
    print(result)
    return result
...
def push_wx(_sckey, desp=""):
    if _sckey == "":
        ...
        params = {"text": "小米运动 步数修改", "desp": desp}
...
def push_server(_sckey, desp=""):
    if _sckey == "":
        ...
        params = {"title": "小米运动 步数修改", "desp": desp}
...
def push_tg(token, chat_id, desp=""):
    if token == "":
        ...
        params = {"text": "小米运动 步数修改\n\n" + desp, "chat_id": chat_id}
